 ...
 * 生成随机的 UUID
 */
export function genUUID(prefix: string, randomLength: number): string {
  const content = Number(Math.random().toString().substring(3, randomLength) + Date.now()).toString(36);
