    ...
    await this.validationService.validationToken(this.token).then((r: any) => {
      if (r) {
        if (r.code === '') {
          ...
          this.validTokenMsg = 'common.message.error.E_032';
        } else if (r.code === 'parseTokenErr') {
          this.validTokenMsg = 'common.message.error.E_033';
        } else if (r.code === 'expiredTokenErr') {
          this.validTokenMsg = 'common.message.error.E_036';
        } else {
    ...
    this.userService.setNewPassword(this.token, newPassword).then((r: any) => {
      if (r) {
        if (r.code === 'nullTokenErr') {
          // 空令牌
          this.message.error(this.i18n.translateLang('common.message.error.E_032'));
          return;
        }
        if (r.code === 'parseTokenErr') {
          // 无效令牌
          this.message.error(this.i18n.translateLang('common.message.error.E_033'));
          return;
        }
        if (r.code === 'expiredTokenErr') {
          // 过期令牌
          this.message.error(this.i18n.translateLang('common.message.error.E_036'));
          return;
        }
