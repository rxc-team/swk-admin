 ...
 * @Descripttion: 登录二次验证
 * @Author: Rxc 陳平
 * @Date: 2020-08-07 17:10:38
 * @LastEditors: Rxc 陳平
 * @LastEditTime: 2020-08-27 17:20:31
 */
        ...
        <form nz-form [formGroup]="validateForm">
          <nz-form-item>
            <nz-form-label nzRequired="true">
              {{ 'menu.secondCheckCaptcha' | translate }}
            </nz-form-label>
            <nz-form-control nzSpan="12" [nzErrorTip]="captcha">
              <nz-input-group>
                <input
                  ...
                  [placeholder]="'page.login.placeholder.captchaPlaceholder' | translate"
                  minlength="6"
                />
              </nz-input-group>
            ...
            <nz-form-control nzSpan="5">
              ...
              <div style="display: inline-block;width: 120px;font-size: 12px;" *ngIf="countTime === 0">
                <span style="color: red;margin-left: 12px;">{{ 'page.login.validator.expired' | translate }}</span>
              </div>
            </nz-form-control>
          </nz-form-item>
        </form>
  ...
  ngOnInit(): void {
    ...
    this.captchaService.getSecondCaptcha(this.user.customer_id, this.user.user_id, this.user.email, this.user.notice_email).then(res => {
      if (res) {
        ...
        // 无法发送验证码
        this.message.error(this.i18n.translateLang('common.message.error.E_035'));
      }
    });
  }
