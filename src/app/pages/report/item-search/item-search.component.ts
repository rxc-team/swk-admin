...
import { DatabaseService, FieldService, ItemService, OptionService, UserService } from '@api';
...
export class ItemSearchComponent implements OnInit, OnChanges {
  ...
  loading = false;
  searchValue = '';
  optionList: Map<string, Array<any>> = new Map();
   ...
   */
  reflect() {
    const hasSelect = this.dataSet.find(item => item[1] === true);
    if (hasSelect) {
      if (this.clear) {
        this.dataSet = [];
      }
      this.nxReflect.emit({ index: this.index, value: this.lookItem });
    }
  }
