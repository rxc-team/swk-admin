/*
 * @Description: 应用程序--多语言管理
 * @Author: RXC 陳平
 * @Date: 2020-06-15 15:25:13
 * @LastEditors: RXC 陈辉宇
 * @LastEditTime: 2020-09-23 14:22:25
 */
...
@Component({
  selector: 'app-languge-apps',
  templateUrl: './languge-apps.component.html'
})
export class LangugeAppComponent implements OnInit, OnChanges {
  langCols = [
    {
  ...
  // 项目多语言重名监测用
  listOfDataSet: LanguageData[] = [];
  allLangMap: { [key: string]: LanguageData[] } = {};
  // 所属项目
  subProject = 'apps';
  ...
  async search() {
    ...
    if (this.zhData.apps) {
      ...
      for (const appId in this.zhData.apps) {
        ...
        ld.itemName = this.zhData.apps[appId].app_name;
        this.listOfDataSet.push(ld);
        if (this.appId === appId) {
          this.listOfData.push(ld);
        }
      }
      this.langMap['zh-CN'] = this.listOfData;
      this.allLangMap['zh-CN'] = this.listOfDataSet;
      this.listOfData = [];
      this.listOfDataSet = [];
    } else {
      this.langMap['zh-CN'] = this.listOfData;
    ...
    if (this.enData.apps) {
      ...
      for (const appId in this.enData.apps) {
        ...
        ld.itemName = this.enData.apps[appId].app_name;
      ...
      this.allLangMap['en-US'] = this.listOfDataSet;
      this.listOfData = [];
      this.listOfDataSet = [];
    } else {
      this.langMap['en-US'] = this.listOfData;
    ...
    if (this.jaData.apps) {
      ...
      for (const appId in this.jaData.apps) {
        ...
        ld.itemName = this.jaData.apps[appId].app_name;
      ...
      this.allLangMap['ja-JP'] = this.listOfDataSet;
      this.listOfData = [];
      this.listOfDataSet = [];
    } else {
      this.langMap['ja-JP'] = this.listOfData;
    ...
    if (this.thData.apps) {
      ...
      for (const appId in this.thData.apps) {
        ...
        ld.itemName = this.thData.apps[appId].app_name;
      ...
      this.allLangMap['th-TH'] = this.listOfDataSet;
      this.listOfData = [];
      this.listOfDataSet = [];
    } else {
      this.langMap['th-TH'] = this.listOfData;
  ...
  updateEditCache(lang: string): void {
    ...
    if (this.listOfData) {
      this.mainLangData.forEach(it => {
        let itemdata = this.listOfData.find(item => item.itemId === it.itemId);
        if (!itemdata) {
          const ld: LanguageData = {
            itemId: it.itemId,
            itemName: ''
    ...
    this.langMap[lang] = this.listOfData;
